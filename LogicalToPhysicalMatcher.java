...
public class LogicalToPhysicalMatcher{
	
	private static final Log log = LogFactory.getLog(LogicalToPhysicalMatcher.class);
	...
	public void init() {
		...
		log.debug("Begin Parsing of Logical To Physical Matching file.");
		
		try
		{
			String pathFile = config.getProperty("huawei.m2v2r0.ran.static.file.logical.to.physical.mapper");
			if(null == pathFile)
			{
				log.error("Missing Attribute (huawei.m2v2r0.ran.static.file.logical.to.physical.mapper) in context file.");
				return;
			}
		...
		log.info("End of Parsing of Logical To Physical Matching file.");
	}
